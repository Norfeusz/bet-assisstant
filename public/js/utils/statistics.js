...
export function calculateTeamStatistics(teamName, matches, leagueFilter = null) {
	...
	const percentAboveCornersForMode =
		cornersForArray.length > 0 ? ((matchesAboveCornersForMode / cornersForArray.length) * 100).toFixed(1) : 'N/A'
	...
	return {
		...
		mostFrequentCornersFor:
			cornersForMode.value !== 'N/A' ? `${cornersForMode.value} (${cornersForMode.count}x)` : 'N/A',
		mostFrequentCornersAgainst:
			cornersAgainstMode.value !== 'N/A' ? `${cornersAgainstMode.value} (${cornersAgainstMode.count}x)` : 'N/A',
