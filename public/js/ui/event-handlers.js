...
export async function loadLeagues(country) {
    ...
    try {
        ...
        const html = leagues.map(league => `
            <div class="league-item ${state.configuredLeagues.has(league.id) ? 'selected' : ''}" id="league-${league.id}">
                <input 
                    type="checkbox" 
                    class="league-checkbox" 
                    ${state.configuredLeagues.has(league.id) ? 'checked' : ''}
                    onchange="window.toggleLeague(${league.id}, '${league.name}', '${country}', '${league.type}', this.checked)"
                >
